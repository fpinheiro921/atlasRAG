...
export const SpecDisplay: React.FC<SpecDisplayProps> = ({ specContent, isLoading, onSaveSpec, onSpecContentChange, isLoggedIn, currentSpecId, userProfile, generationsRemaining, onGenerationUsed }) => {
  ...
  const handleSendElaborationQuestion = async () => {
    ...
    if (generationsRemaining !== null && generationsRemaining <= 0) {
        setElaborationError("You've reached your monthly generation limit.");
        return;
    }
  ...
  const handleAnalyzeSpec = useCallback(async () => {
    ...
    try {
      ...
      // This is a premium action, so we count it as a generation.
      await onGenerationUsed();
    } catch (err: any) {
  ...
  }, [specContent, parseAndStructureAnalysis, parsedSections, onGenerationUsed]);
  ...
  const handleSendRegenerationRequest = async () => {
    ...
    if (generationsRemaining !== null && generationsRemaining <= 0) {
        setRegenerationError("You've reached your monthly generation limit.");
        return;
    }
  ...
  const analyzeButtonDisabled = !specContent || isLoading || isAnalyzingSpec || !isLoggedIn || (generationsRemaining !== null && generationsRemaining <= 0);
  const analyzeButtonTitle = !isLoggedIn 
      ...
      : "Analyze Full Spec with AI (uses 1 generation)";
                ...
                {activeSection && !isLoading && (
                  <div className="mb-6 flex flex-wrap justify-end gap-2 -mt-2">
                    <button
                      ...
                      title={isLoggedIn ? `Regenerate "${activeSection.title}" with AI (uses 1 generation)` : "Log in to regenerate section"}
                      disabled={!isLoggedIn || (generationsRemaining !== null && generationsRemaining <= 0)}
                    >
                    ...
                    <button
                      ...
                      title={isLoggedIn ? `Ask a follow-up question about "${activeSection.title}" (uses 1 generation)` : "Log in to elaborate"}
                      disabled={!isLoggedIn || (generationsRemaining !== null && generationsRemaining <= 0)}
                    >
          ...
          >
            ...
            <div className="p-6 space-y-4 overflow-y-auto custom-scrollbar flex-1">
              ...
              >
                ...
                {isElaborating ? 'Thinking...' : 'Ask AI for Elaboration (uses 1 generation)'}
              </button>
          ...
          >
            ...
            <div className="p-6 space-y-4 overflow-y-auto custom-scrollbar flex-1">
              ...
              >
                ...
                {isRegenerating ? 'Regenerating...' : 'Regenerate with AI (uses 1 generation)'}
              </button>
