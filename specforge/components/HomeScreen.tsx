...
import { Feature1InputVisual } from './landing/Feature1InputVisual';
import { Feature2AiCraftsVisual } from './landing/Feature2AiCraftsVisual';
import { Feature3ExportVisual } from './landing/Feature3ExportVisual';

interface HomeScreenProps {
  onGetStarted: () => void;
  onLogin: () => void;
  currentUser: firebase.User | null;
  onNavigateToBuildingInPublic: () => void;
}
...
const FeatureSection: React.FC<FeatureSectionProps> = ({ number, title, description, visualSide, iconName, visual }) => (
...
export const HomeScreen: React.FC<HomeScreenProps> = ({ onGetStarted, onLogin, currentUser, onNavigateToBuildingInPublic }) => {
  ...
  return (
    <div className="min-h-screen bg-base text-text-body flex flex-col items-center p-4 sm:p-6 lg:p-8 selection:bg-accent selection:text-white antialiased">
      ...
      <header className="text-center py-16 lg:py-24 w-full max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button
                ...
                className="w-full sm:w-auto bg-accent text-white font-semibold rounded-lg px-10 py-4 text-lg hover:bg-accent-dark focus:outline-none focus:ring-4 focus:ring-accent/50 transition-all duration-150 ease-in-out transform hover:scale-105 shadow-xl hover:shadow-accent/40 flex items-center justify-center"
            >
                <span className="material-symbols-outlined mr-2">auto_fix_high</span>
                {currentUser ? 'Start Forging' : 'Forge Your First Spec'}
            </button>
            <button
                onClick={onNavigateToBuildingInPublic}
                className="w-full sm:w-auto border border-text-muted text-text-muted font-semibold rounded-lg px-10 py-4 text-lg hover:bg-panel hover:text-text-body focus:outline-none focus:ring-4 focus:ring-accent/50 transition-all duration-150 ease-in-out flex items-center justify-center"
            >
                <span className="material-symbols-outlined mr-2">description</span>
                Read the Free Guide
            </button>
        </div>
        <p className="text-sm text-text-muted mt-4">Get started instantly. No credit card required.</p>
      </header>
      ...
      <footer className="mt-20 py-8 text-center text-text-muted text-sm border-t border-panel w-full max-w-6xl mx-auto flex flex-col sm:flex-row justify-center items-center gap-2">
